...
func (mac MAC) MarshalYAML() (interface{}, error) {
	return mac.String(), nil
}

func (mac *MAC) UnmarshalYAML(node *yaml.Node) error {
	str, err := strconv.Unquote(string(node.Value))
	if err != nil {
		return mac.UnmarshalText([]byte(node.Value))
	}
	return mac.UnmarshalText([]byte(str))
}
